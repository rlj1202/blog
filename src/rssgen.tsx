...
export const generateRssFeed = async (articleAndContents: Article[]) => {
  ...
  articleAndContents.forEach((article) => {
    feed.addItem({
      ...
      contributor: [],
      date: new Date(article.date) || date,
      // image: '',
    });
  });
  ...
  fs.mkdirSync(`./public${path.dirname(Config.rss.rss2Path)}`, {
    recursive: true,
  });
  fs.mkdirSync(`./public${path.dirname(Config.rss.atom1Path)}`, {
    recursive: true,
  });
  fs.mkdirSync(`./public${path.dirname(Config.rss.json1Path)}`, {
    recursive: true,
  });
